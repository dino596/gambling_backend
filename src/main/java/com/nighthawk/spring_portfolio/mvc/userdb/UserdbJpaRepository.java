...
/**
 * The UserdbJpaRepository interface is automatically implemented by Spring Data JPA at runtime.
 ...
 * 1. The entity type to be persisted (in this case, Userdb).
...
public interface UserdbJpaRepository extends JpaRepository<Userdb, Long> {
    ...
    List<Userdb> findAllByOrderByNameAsc();
    List<Userdb> findByNameContainingIgnoreCaseOrEmailContainingIgnoreCase(String name, String email);
    Userdb findByEmailAndPassword(String email, String password);
     ...
     * The query will find all Userdb entities where the name or email contains the given term.
     * The 'nativeQuery = true' parameter indicates that the query is a native SQL query, not a JPQL query.
     */
    @Query(
            value = "SELECT * FROM Userdb p WHERE p.name LIKE ?1 or p.email LIKE ?1",
            nativeQuery = true)
    List<Userdb> findByLikeTermNative(String term);
}